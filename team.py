...
def generate_random_word():
    word_list = [
        "TmrTaitan080", "TmrTaitan", "TaitanTeam", "VsTeam", "TmrMod", "Tmr080", "Taitan080", "TmrVip777"
    ]
...
def get_jwt_token():
    global jwt_token
    url = "https://dark-teal.vercel.app/token?uid=4046916457&password=53A01F233F08B3BABCFE74F1C563BE4D2D2BB8FF7A28178C04A99A2CCE519C47"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Get token directly from the response
            if "token" in data:
                ...
                print("[!] 'token' field not found in the response.")
        else:
...
class FF_CLIENT(threading.Thread):
    def __init__(self, id, password, target_uid, team_number):
        super().__init__()
        self.id = id
    ...
    def sockf1(self, tok, host, port, packet, key, iv):
        ...
        sleep(1)
        packetfinal = self.changes(self.team_number)
        socket_client.send(packetfinal)
        moi = self.invite_skwad(self.target_uid)
        socket_client.send(moi)
        moi1 = self.invite_skwad(self.target_uid)
        socket_client.send(moi1)
        logging.info(f"Đã Mời {self.target_uid} Thành Công")
        sleep(9)
        leavee = self.leave_s()
        socket_client.send(leavee)
        sleep(1)
        leaveee = self.leave_s()
        socket_client.send(leaveee)
...
def map_team_number(team_input):
    team_mapping = {
        3: 2,
        4: 3,
        5: 4,
    ...
    return team_mapping.get(team_input, 4)

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("To Make a Bot, Write Code for the Bot to Call a Function and Run This Command: python team.py uid-needs-to-invite-team-number")
        sys.exit(1)
    try:
        target_uid = int(sys.argv[1])
        team_input = int(sys.argv[2])
        if team_input not in [3, 4, 5, 6]:
            print("team phải là 3, 4, 5 hoặc 6")
            sys.exit(1)
        team_number = map_team_number(team_input)
        logging.info(f"Đang Mời {target_uid} Vào Team {team_number}")
    except ValueError:
        sys.exit(1)
    try:
